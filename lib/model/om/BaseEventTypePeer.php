...
/**
 * Base static class for performing query and update operations on the 'events_type' table.
 *
 * 
 *
...
abstract class BaseEventTypePeer {
	...
	const ID = 'events_type.ID';

	/** the column name for the NAME field */
	const NAME = 'events_type.NAME';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseEventTypePeer', $criteria, $con);
		}
	...
	/**
	 * Method to invalidate the instance pool of all tables related to events_type
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEventTypePeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseEventTypePeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEventTypePeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseEventTypePeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEventTypePeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseEventTypePeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEventTypePeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseEventTypePeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the events_type table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseEventTypePeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
