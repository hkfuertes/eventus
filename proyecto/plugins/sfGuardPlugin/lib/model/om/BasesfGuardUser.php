...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Tokens[] Collection to store aggregation of Tokens objects.
	 */
	protected $collTokenss;

	/**
	 * @var        Criteria The criteria used to select the current contents of collTokenss.
	 */
	private $lastTokensCriteria = null;
	...
	/**
	 * Clears out the collTokenss collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collTokenss collection (array).
	 *
	 ...
	 * @return     void
	 */
	public function initTokenss()
	{
		$this->collTokenss = array();
	}

	/**
	 * Gets an array of Tokens objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Tokenss from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getTokenss($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collTokenss === null) {
			if ($this->isNew()) {
			   $this->collTokenss = array();
				...
				$criteria->add(TokensPeer::USER_ID, $this->id);

				TokensPeer::addSelectColumns($criteria);
				$this->collTokenss = TokensPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				TokensPeer::addSelectColumns($criteria);
				if (!isset($this->lastTokensCriteria) || !$this->lastTokensCriteria->equals($criteria)) {
					$this->collTokenss = TokensPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastTokensCriteria = $criteria;
	 ...
	 * @throws     PropelException
	 */
	public function countTokenss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collTokenss === null) {
			if ($this->isNew()) {
				...
				$count = TokensPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Tokens object to this object
	 * through the Tokens foreign key attribute.
	 *
	 * @param      Tokens $l Tokens
	 * @return     void
	 * @throws     PropelException
	 */
	public function addTokens(Tokens $l)
	{
		if ($this->collTokenss === null) {
			$this->initTokenss();
		}
		if (!in_array($l, $this->collTokenss, true)) { // only add it if the **same** object is not already associated
			array_push($this->collTokenss, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collTokenss) {
				foreach ((array) $this->collTokenss as $o) {
					$o->clearAllReferences($deep);
				}
			}
